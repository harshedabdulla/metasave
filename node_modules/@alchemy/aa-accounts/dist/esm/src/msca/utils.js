import { encodeAbiParameters, encodeFunctionData, encodeFunctionResult, keccak256, parseAbiParameters, } from "viem";
...
export const getDefaultMultiOwnerMSCAFactoryAddress = (chain, excludeDefaultTokenReceiverPlugin = false) => {
    switch (chain.id) {
        case sepolia.id:
...
export const getMSCAUpgradeToData = async (provider, multiOwnerPluginAddress, tokenReceiverPluginAddress) => {
    const factoryAddress = getDefaultMultiOwnerMSCAFactoryAddress(provider.rpcClient.chain);
    ...
    const hashedTrPluginManifest = keccak256(encodeFunctionResult({
        abi: IPluginAbi,
        functionName: "pluginManifest",
    ...
    const encodedPluginInitData = encodeAbiParameters(parseAbiParameters("bytes32[], bytes[]"), [
    ...
    return {
        ...
        connectFn: (rpcClient) => createMultiOwnerMSCA({
            rpcClient,
            owner,
