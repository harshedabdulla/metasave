...
export const InstallMethodGenPhase = async (input) => {
    ...
    if (config.installConfig != null) {
        ...
        addType(`Install${contract.name}Params`, dedent `{
        args: Parameters<typeof encodeAbiParameters<InstallArgs>>[1];
        pluginAddress?: Address;
        dependencyOverrides?: FunctionReference[];
    }`, true);
        const dependencies = (config.installConfig.dependencies ?? []).map((x) => dedent `
        (() => {
