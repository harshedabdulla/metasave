...
import { buildTreeState, isValidOperation, QueryOperators } from '../circuits';
import { buildFieldPath } from '../verifiable';
...
export const parseQueryMetadata = async (propertyQuery, ldContextJSON, credentialType, options) => {
    const datatype = await Path.newTypeFromContext(ldContextJSON, `${credentialType}.${propertyQuery.fieldName}`, options);
    const query = {
        ...propertyQuery,
        slotIndex: 0,
    ...
    const serAttr = await Parser.getSerializationAttrFromContext(JSON.parse(ldContextJSON), options, credentialType);
    if (!serAttr) {
    ...
    if (!query.merklizedSchema) {
        query.slotIndex = await Parser.getFieldSlotIndex(propertyQuery.fieldName, credentialType, byteEncoder.encode(ldContextJSON));
    }
    else {
        try {
